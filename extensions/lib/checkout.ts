...
export async function checkoutOf(
  ...
  return parseCheckout(await git(CHECKOUT_ARGS, nearestExistingDir(absPath)));
}

// Never throws: any failure (not a repo, missing dir, old git) is a nonzero code.
export const gitSync = (args: string[], cwd: string): GitResult => {
  try {
    const stdout = execFileSync("git", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"], timeout: 5000 });
