...
- `spec-council-member` — adversarial single-model spec critic; one per configured council model. Emits an `over-spec` finding for clauses outside the problem, unrequested by any human input, and unnecessary to deliver; every report closes with a mandatory `lean:` line; every finding ends with `probed: <source or check> - <result>` or `probed: none`. Dispatched by `roasting-the-spec` and `shape-ticket` (the latter at `:low` thinking via a model-suffix override, for ticket roasts), and by brainstorming's `reference/amendment-surface.md` in `Mode: amendment-review` (a task-text discriminator; one `auto-apply | escalate` line per proposed spec amendment) on the main loop's own model and reasoning level.
- `spec-council-synthesizer` — neutral chair that consolidates and adjudicates member critiques. Preserves `over-spec:` as a cluster prefix (an over-spec cluster loses only to a finding proving the clause load-bearing) and tallies `lean:` across members; tags each suggested edit `grounded:` (member probes support every assertion) or `hypothesis:`. Dispatched only by `roasting-the-spec` and `shape-ticket` (the latter at `:low` thinking via a model-suffix override, for ticket roasts).
