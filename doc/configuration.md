...
A tool, not a hook. Skills call `plan_tracker({ action: "init" | "add" | "update" | "status" | "clear", ... })` to manage a task list; a TUI widget above the editor shows progress (✓/→/○). State branches with the session, no config needed. `add` appends genuinely new planned work as pending, preserving existing task statuses. Review and whole-diff repairs reopen the owning existing plan-task indices instead of creating fix sub-wave entries; conformance remediation uses its durable `G<n>` entries, reusing the exact existing gap before adding a new one. Statuses: `pending ○ / in_progress → / complete ✓ / failed ✗` (failed renders in error color). `failed` is terminal-negative: the task ran and did not pass; excluded from the complete count; never selected as the current task in the widget. Additive - no settings key, no migration; existing serialized sessions unaffected.
