...
A recorder, not a tool. Inside a gauntlet flow it writes one YAML record per spec path at the toplevel of the checkout containing the spec (a worktree spec commits on the worktree branch even when pi runs in the primary), under `<dir>/<spec path with .md replaced by .yaml>` (default `.pi/gauntlet/telemetry/doc/specs/<spec>.yaml`) and commits it on the current branch with pathspec commits (`git commit -q -m "telemetry: <spec>" -- <record>`) at checkpoints: phase start/complete/skip/reset, `plan_check` pass, ship, discard, session shutdown. Only the parent session writes (children with `PI_SUBAGENT_DEPTH` >= 1 no-op); child usage arrives through the parent's `subagent` results.
...
Ship: the first `git merge --squash` / `git push` / `gh pr create` (`git -C <path>` global flags are accepted) while `ship` is in progress sets `shipped_at`, computes the diff summary, commits, then runs; a non-zero exit appends `ship_failed` and clears the shipped state so a retry can succeed. `git worktree remove` / `git branch -D` during ship records `abandoned`. `phase_tracker complete ship` with no ship command (keep the branch) also sets `shipped_at`.
...
It reads `piGauntlet.telemetry` from the same two layers as the recorder (preset `$PI_CODING_AGENT_DIR/settings.json`, default `~/.pi/agent`, under `<toplevel>/.pi/settings.json`; `--dir` overrides and is validated by the same resolver), resolves the base (`--base`, else `origin/HEAD`, `main`, `master`), lists `doc/specs/*.md` files changed on the branch, and for each record `<dir>/<spec>.yaml` prints exactly one line: `present <path>`; `present <path> (marked shipped)` (the record was `in_progress` with no `derived.phases.ship` - the recorder lost its binding before the ship - and the salvage committed `status: shipped` + `shipped_at` as `telemetry: mark <path> shipped at landing`; a worktree or index copy differing from HEAD is left alone); `restored <path> from <sha>` (the deleting commit - the record is taken from its parent, or from the on-disk copy when the recorder rewrote it, and committed as `telemetry: restore record stripped in <sha[0:10]>`); `restored <path> from <sha> (marked shipped)` (the same stamp applied to the restored bytes inside the single restore commit); `no telemetry run <path>` (no `telemetry: ` commit on the branch); `never written <path>` (including when the branch contains the commit that introduced the record but no later deletion); `stripped <path> in <sha>` (with `--check`, detect-only); `unfinished <path>` (with `--check`: the record would be stamped); `restore failed <path>: <reason>` (including `staged copy differs from worktree` for a pre-staged record whose worktree copy differs or is missing, hook rejection, or a 30 s commit timeout; the script rolls back only its own changes). Bare outcomes: `telemetry disabled`, `no base ref`, `no spec on branch`. Every valid invocation exits 0 and the script never pushes. The stamp never re-derives: `derived` stays as recorded, so a stamped record shows its missing phases to consumers (the `gauntlet-performance` digest marks it `shipped*`). Malformed invocation, including an invalid `--dir`, prints usage and exits 1.

Call sites: `finishing-a-development-branch` runs it on the feature branch after the plan strip and before the Option 3 `git merge --squash` (the plan strip now happens on the branch, not on the primary's staged index) or the Option 1/2 `git push`; `gatekeep-pr` runs `--check` at assessment (a `stripped` result is a blocking `P#`), runs it after every fix wave before that wave's push, and runs it as the first step of a selected merge course - the one case where the gate pushes (that single `telemetry: restore` commit) and merges in the same selection.
